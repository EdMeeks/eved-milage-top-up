...
const validVehicleRecords = [
  { registration: 'AB25CDE', v5cReference: '01234567890' },
  { registration: 'CD25EFG', v5cReference: '10234567890' },
  { registration: 'AB75CDE', v5cReference: '12345678901' },
  { registration: 'CD75EFG', v5cReference: '23456789012' },
  { registration: 'EF75GHI', v5cReference: '34567890123' },
  { registration: 'GH75JKL', v5cReference: '45678901234' },
  { registration: 'JK75MNO', v5cReference: '56789012345' },
  { registration: 'LM75NPR', v5cReference: '67890123456' },
  { registration: 'MN26PQR', v5cReference: '78901234567' },
  { registration: 'NP26RST', v5cReference: '89012345678' },
  { registration: 'PR26STU', v5cReference: '90123456789' },
  { registration: 'RS26TUV', v5cReference: '11234567890' },
  { registration: 'ST26UVW', v5cReference: '22345678901' },
  { registration: 'TU26VWX', v5cReference: '33456789012' }
]

const ineligibleVehicleRecords = [
  { registration: 'UV25XYZ', v5cReference: '44567890123' },
  { registration: 'VW75XYZ', v5cReference: '55678901234' },
  { registration: 'WX26YZA', v5cReference: '66789012345' },
  { registration: 'XY26ZAB', v5cReference: '77890123456' }
]

const taxDueVehicleRecords = [
  { registration: 'ZA25BCD', v5cReference: '88901234567' },
  { registration: 'ZB75CDE', v5cReference: '99012345678' },
  { registration: 'ZC26DEF', v5cReference: '10123456789' },
  { registration: 'ZD26EFG', v5cReference: '21123456789' }
]

function normaliseVehicleRegistration(input) {
  return String(input || '').toUpperCase().replace(/[^A-Z0-9]/g, '')
}

function normaliseV5cReference(input) {
...
if (mileageUnit === 'miles' && cleanedMiles !== '') {
  ...
  if (numericMiles < PREVIOUS_RECORDED) {
    return res.redirect('/v2/b-eVED-screens/do-you-know-exact-mileage-error')
  }

  return res.redirect('/v2/b-eVED-screens/mileage-balance')
}
...
router.get("/a-starting-screens/start-page", function (request, response) {
  ...
  return response.render("v2/a-starting-screens/start-page", { data: request.session.data });
});
...
router.get("/a-starting-screens/do-you-have-a-V5c", function (request, response) {
  ...
  return response.render("v2/a-starting-screens/do-you-have-a-V5c", { data: request.session.data });
});

router.post("/v2/a-starting-screens/do-you-have-a-V5c-answer", function (request, response) {
  if (request.session.data["v5c-radios"] === "yes") {
    return response.redirect("/v2/a-starting-screens/V5c-number");
  }

  return response.redirect("/v2/a-starting-screens/which-document-do-you-have");
});

//Doesn't have V11 reminder letter:
router.post("/a-starting-screens/eved-options-screen", function (request, response) {
  if (request.session.data["eved-options"] === "yes") {
    return response.redirect("/v2/a-starting-screens/tax-start-page");
  }

  return response.redirect("/v2/b-eVED-screens/do-you-know-exact-mileage");
});
...
router.post('/a-starting-screens/which-document-do-you-have-answer', function (req, res) {
  ...
  if (documentType === 'v5c-2') {
    return res.redirect('/v2/a-starting-screens/v5c-2')
  }

  if (documentType === 'no-docs') {
    return res.redirect('/v2/a-starting-screens/no-docs')
  }
...
router.post('/a-starting-screens/vehicle-record-answer', function (req, res) {
  const source = req.body.source === 'v5c-2' ? 'v5c-2' : 'v5c-number'
  const registrationField = source === 'v5c-2' ? 'vehicle-reg-2' : 'vehicle-reg'
  const registration = normaliseVehicleRegistration(req.body[registrationField])
  const v5cReference = normaliseV5cReference(req.body['v5c-ref'])
  const matchingRecord = validVehicleRecords.find(function (record) {
    return record.registration === registration && record.v5cReference === v5cReference
  })
  const matchingIneligibleRecord = ineligibleVehicleRecords.find(function (record) {
    return record.registration === registration && record.v5cReference === v5cReference
  })
  const matchingTaxDueRecord = taxDueVehicleRecords.find(function (record) {
    return record.registration === registration && record.v5cReference === v5cReference
  })

  req.session.data[registrationField] = formatNumberPlate(registration)
  req.session.data['vehicle-reg'] = formatNumberPlate(registration)
  req.session.data['v5c-ref'] = v5cReference
  req.session.data.vehicleTaxDue = Boolean(matchingTaxDueRecord)

  if (matchingIneligibleRecord) {
    return res.redirect('/v2/a-starting-screens/vehicle-not-currently-eligible')
  }

  if (!matchingRecord && !matchingTaxDueRecord) {
    return res.redirect('/v2/a-starting-screens/unable-to-find-vehicle-records')
  }

  req.session.data.registrationEntryPath = source
  return res.redirect('/v2/a-starting-screens/check-vehicle-details?vehicle-reg=' +
    encodeURIComponent(formatNumberPlate(registration)) + '&source=' + source)
})
...
router.post("/b-eVED-screens/vehicle-details", function (request, response) {
  ...
  if (request.session.data.vehicleTaxDue) {
    return response.redirect("/v2/a-starting-screens/eved-options");
  }
...
function handleMileageChoice(request, response, noDestination, fromPaymentSummary) {
  ...
  if (addMoreMiles === "yes") {
    return response.redirect("/v2/b-eVED-screens/estimate-your-miles?mileage=" + encodeURIComponent(currentMileage || ""));
  }
...
router.post("/b-eVED-screens/mileage-balance-answer", function (request, response) {
  const destination = "/v2/b-eVED-screens/no-extra-mileage";
  const currentMileage = request.body.currentMileage || request.session.data["exactMileageMiles"];
  const isInDebit = Number(currentMileage) > 36825 + 8000;

  if (request.body["add-more-miles"] === "no") {
    if (isInDebit) {
      return response.redirect("/v2/b-eVED-screens/your-payment-summary?currentMileage=" + encodeURIComponent(currentMileage || ""));
    }
...
router.post("/b-eVED-screens/your-payment-summary-answer", function (request, response) {
  if (!request.body["add-more-miles"] && Number(request.session.data.estimatedMileage) > 0) {
    return response.redirect("/v2/c-payment-screens/confirmation-of-your-payment");
  }

  return handleMileageChoice(request, response, "/v2/c-payment-screens/confirmation-of-your-payment", true);
});
...
router.get('/b-eVED-screens/estimate-your-miles', function (req, res) {
  ...
  return res.render('v2/b-eVED-screens/estimate-your-miles', { data: req.session.data })
})
...
router.post('/b-eVED-screens/estimate-your-miles-answer', function (req, res) {
  ...
  if (estimatedMileage < minimumMileage) {
    ...
    return res.render('v2/b-eVED-screens/estimate-your-miles', {
      error: true,
  ...
  return res.redirect('/v2/b-eVED-screens/your-payment-summary?currentMileage=' + encodeURIComponent(currentMileage || ''))
})
...
router.get('/b-eVED-screens/your-payment-summary', function (req, res) {
  ...
  if (req.query.remove === 'true') {
    ...
    if (req.query.redirect === 'mileage-balance' && Number(currentMileage) < purchasedMileage) {
      return res.redirect('/v2/b-eVED-screens/mileage-balance?mileage=' + encodeURIComponent(currentMileage || ''))
    }
  }
  ...
  // Render the page
  return res.render('v2/b-eVED-screens/your-payment-summary', {
    data
  })
})
...
const ddTwiceYearly = total / 2
  ...
  return res.render('v2/c-payment-screens/confirmation-of-your-payment', { data })
})
...
router.post('/c-payment-screens/confirmation-of-your-payment', function (req, res) {
  ...
  if (emailConfirmation === 'yes') {
    ...
    return res.redirect('/v2/c-payment-screens/email-confirmation')
  }

  if (emailConfirmation === 'no') {
    data['email-confirmation'] = emailConfirmation
    return res.redirect('/v2/c-payment-screens/card-payment-details')
  }

  // Nothing selected -> show error
  data.emailConfirmationError = true
  return res.redirect('/v2/c-payment-screens/confirmation-of-your-payment')
})

router.get('/c-payment-screens/email-confirmation', function (req, res) {
  return res.render('v2/c-payment-screens/email-confirmation', { data: req.session.data })
})
...
router.get('/c-payment-screens/card-payment-details', function (req, res) {
  return res.render('v2/c-payment-screens/card-payment-details', { data: req.session.data })
})

// POST – no validation required, always continue
router.post('/c-payment-screens/card-payment-details', function (req, res) {
  return res.redirect('/v2/d-confirmation-screens/card-confirmation-page')
})
